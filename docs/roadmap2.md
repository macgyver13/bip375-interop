...
`spdk-cli/Cargo.toml` hardcoded absolute paths into the local spdk and rust-psbt checkouts (now git dependencies pinned by `rev`). `config/interop.example.yaml` hardcoded the same home directory (now `~`). `rust-psbt` is not in `interop.lock`.
